...
harness and Discord. It is intentionally not a runner, bridge, Discord bot, or
control plane.

```text
Discord <-> native Hermes/OpenClaw gateway <-> existing agent
                                           \
...
Checked on 2026-08-27 in this checkout. The probe recorded executable presence,
...
| --- | --- | --- | --- |
| Hermes Agent | v0.20.6 installed; Work Map remote MCP discovers all six tools; local Codex MCP and the Master Skill are enabled | Native Discord gateway and remote HTTP MCP client; MCP tools are discovered and exposed to platform toolsets | Add `DISCORD_BOT_TOKEN`, bind the selected channel to the Skill's frontmatter name `master` with both mention gates, add guild/channel/user allowlists, then prove one explicit mention/reply |
| pi agent | `pi` not found; `~/.pi` absent | Skills/context are native, but the verified core capability set has neither MCP nor a Discord gateway | Not a V1 ready path; choose a harness with native MCP and native channel delivery |
...
| Discord | Developer Portal is open but not signed in; Hermes reports only `DISCORD_BOT_TOKEN` missing for its Discord tools | Discord Developer Portal + Gateway are the native integration surface | Sign in, create/invite the bot with least privilege, set the token and allowlists, then run the native mention/reply proof |

These results are a local 2026-08-27 smoke test, not a portable credential
...
Hermes/Discord it also checks the selected `DISCORD_CHANNEL_ID` is explicitly
bound to a Master skill in `discord.channel_skill_bindings` with both mention
gates enabled.
...
merge the exact [Hermes Discord binding template](../examples/mcp/hermes-discord-work-map.yaml), replace its selected channel ID, and keep both
`require_mention: true` and `thread_require_mention: true`. Bind `master`, the
...
skill auto-selection or a slash command. Register the resulting Master or
default-agent Discord address in the teammate record; the Work Map service
cannot discover or create it.
...
Pi can load a standard `SKILL.md`, but the verified core capability set does
not include MCP or a Discord gateway. Menoteam deliberately provides no
compatibility adapter or fallback bridge, so pi is not a V1 ready path unless
its core capabilities change. Use Hermes, OpenClaw, or another harness that
natively satisfies both boundaries.
...
3. Configure Gateway intents required by the selected harness. Explicit bot
   mentions are the narrowest V1 trigger; normal guild-message listening may
...
4. Configure the harness's user/guild/channel allowlists and test one explicit
   `@Master` mention and one default-agent mention in the original thread.
5. Record only platform-specific agent addresses (not bot tokens) in the
   appropriate teammate record through the Work Map MCP tool.
